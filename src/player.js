        ...
        Play() {
            ...
            for(let i = 0; i < this._board._size; ++i) {
                for(let j = 0; j < this._board._size; ++j) {
                    ...
                    if(tile.owner == "-") {
                        const playerValues = {"x": 0, "o": 0};
                        for(let symbol in tile._data) {

                            for(let k = 0; k < 4; ++k) {
                                const l = (k + 4) % 8;
                                let value = 0;

                                const free1 = tile._data[symbol].free[k];
                                const free2 = tile._data[symbol].free[l];

                                if(free1 + free2 + 1 >= winningCount) {

                                    const connected1 = tile._data[symbol].connected[k];
                                    const connected2 = tile._data[symbol].connected[l];
                                    ...
                                    if(connected < 3) {
                                        if(connected1 < 3 && free1 > connected1 + 1) {
                                            ...
                                            count1 = connected1 + nextTile._data[symbol].connected[k];
                                        }
                                        if(connected2 < 3 && free2 > connected2 + 1) {
                                            ...
                                            count2 = connected2 + nextTile._data[symbol].connected[l];
                                        }
                                    ...
                                    else if(connected == 2 && bothSidesOpen) value = 75000;
                                    else if(count >= 2 && newBothSidesOpen) value = 50000;
                                    else if(count >= 2) value = 20000;
                                    else if(connected == 1 && bothSidesOpen) value = 20000;
                                    else if(count == 1 && newBothSidesOpen) value = 10000;
                                    else if(connected == 2) value = 2000;
                                    else if(count == 2) value = 1000;
                                    else if(connected == 1) value = 200;
                                    else if(count == 1) value = 100;
                                    else value = free1 + free2 + 1;
                                }

                                playerValues[symbol] += value;

                            }

                        }

                        let resultValue = playerValues[this._symbol] * 1.2 + playerValues[otherSymbol];
